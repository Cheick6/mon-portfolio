...
// Pour que l'application compile sans l'image locale, nous utilisons un lien direct temporaire.
// Une fois le fichier 'photo.jpg' placé dans le dossier src/, vous pourrez décommenter la ligne suivante :
// import profilePic from './photo.jpg';
// const profilePic = "https://ui-avatars.com/api/?name=Cheick+Sow&background=0D8ABC&color=fff&size=200"; 
import profilePic from './photo.jpg';
...
const Portfolio = () => {
  ...
  // --- STYLES DYNAMIQUES (Dark vs Light) ---
  const theme = {
    ...
    shadow: isDarkMode ? 'hover:shadow-cyan-500/10' : 'hover:shadow-cyan-500/20 shadow-sm'
  };
  ...
  const projects = [
    ...
    },
    {
      ...
      type: "HTML/CSS Pur",
      description: "Conception et développement complet d'un site web pour un client fictif (secteur rénovation). Utilisation de HTML5 sémantique et CSS3 pour une mise en page responsive sans framework.",
      tags: ["HTML5", "CSS3", "Responsive Design", "Maquettage"],
      link: "#"
    },
    {
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-cyan-500/30 selection:text-cyan-600 overflow-x-hidden transition-colors duration-500 ${theme.bg} ${theme.text}`}>
      ...
      <div className="fixed inset-0 z-0 pointer-events-none transition-opacity duration-300">
        ...
        ></div>
        <div className={`absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full blur-[120px] animate-pulse-slow ${isDarkMode ? 'bg-blue-900/20' : 'bg-blue-200/40'}`}></div>
        <div className={`absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full blur-[120px] animate-pulse-slow ${isDarkMode ? 'bg-cyan-900/10' : 'bg-cyan-200/40'}`} style={{animationDelay: '2s'}}></div>
      </div>

      {/* Navigation */}
      <nav className={`fixed w-full z-50 top-0 left-0 backdrop-blur-md border-b transition-all duration-300 ${theme.navBg} ${theme.border}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden md:flex items-center gap-6">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 flex items-center gap-2 relative overflow-hidden group ${
                      activeSection === item.id
                        ...
                        : `${theme.textMuted} hover:text-cyan-500 hover:bg-cyan-500/5`
                    }`}
                  >
              ...
              <button 
                onClick={toggleTheme}
                className={`p-2 rounded-full border transition-all duration-300 ${theme.border} ${theme.iconBg} hover:border-cyan-400 text-cyan-500`}
            ...
            <div className="-mr-2 flex items-center gap-4 md:hidden">
              <button 
                onClick={toggleTheme}
                className={`p-2 rounded-full border transition-all duration-300 ${theme.border} ${theme.iconBg} text-cyan-500`}
              >
                {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
              </button>
              <button
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                className={`inline-flex items-center justify-center p-2 rounded-md ${theme.textMuted} hover:text-cyan-500 focus:outline-none border ${theme.border} ${theme.iconBg}`}
              >
        ...
        {/* Mobile menu */}
        {isMenuOpen && (
          <div className={`md:hidden ${theme.navBg} border-b ${theme.border} shadow-xl animate-fade-in-down`}>
      ...
      <main className="relative z-10 pt-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-screen">
        ...
        <section id="home" className="min-h-screen flex flex-col items-center justify-center text-center space-y-8 py-20">
          <RevealOnScroll>
            <div className="relative group inline-block">
              ...
              <div className={`relative rounded-full p-1.5 border-2 w-40 h-40 md:w-56 md:h-56 overflow-hidden ${theme.iconBg} ${isDarkMode ? 'border-cyan-500/30' : 'border-cyan-500/10'}`}>
                {/* Image avec Fallback */}
                <img 
                  src={profilePic} 
                  alt="Cheick-Oumar Sow" 
                  onError={(e) => { e.target.onerror = null; e.target.src = "https://via.placeholder.com/224x224?text=CS"; }}
          ...
          <RevealOnScroll delay={200}>
            <div className="space-y-4 pt-4 max-w-3xl mx-auto">
              ...
              <p className={`text-xl md:text-3xl font-light h-16 md:h-auto ${theme.textMuted}`}>
                {subtitle}<span className="animate-blink text-cyan-500">|</span>
              </p>
              ...
              <div className="flex flex-wrap gap-4 justify-center mt-8 pt-4">
                ...
                <button 
                  ...
                  className={`px-8 py-4 bg-transparent border-2 rounded-full font-bold transition-all hover:-translate-y-1 ${isDarkMode ? 'border-slate-600 hover:border-cyan-400 text-slate-300 hover:text-cyan-400' : 'border-slate-300 hover:border-cyan-600 text-slate-600 hover:text-cyan-600'}`}
                >
                  Me contacter
                </button>
              </div>
        ...
        <section id="about" className="py-20 scroll-mt-20">
          <RevealOnScroll>
            <div className={`flex items-center gap-4 border-b pb-4 mb-12 ${theme.border}`}>
              <BookOpen className="text-cyan-400 animate-bounce-slow" size={32} />
              <h2 className={`text-3xl font-bold ${theme.textTitle}`}>À propos de moi</h2>
            </div>
          </RevealOnScroll>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <RevealOnScroll delay={100}>
              <div className="space-y-6">
                <div className={`${theme.cardBg} p-6 rounded-2xl border ${theme.border} ${theme.cardHover} transition-all shadow-sm group`}>
                  <h3 className={`text-xl font-semibold mb-3 flex items-center gap-2 group-hover:text-cyan-400 transition-colors ${theme.textTitle}`}>
                ...
                <div className={`${theme.cardBg} p-6 rounded-2xl border ${theme.border} ${theme.cardHover} transition-all shadow-sm group`}>
                  ...
                  <p className={`${theme.textMuted} leading-relaxed`}>
                    J'ai découvert la programmation en arrivant à l'IUT. Depuis, j'ai appris à coder avec Java, Python, JS et Flutter à travers de nombreux projets.
        ...
        <section id="skills" className="py-20 scroll-mt-20">
          <RevealOnScroll>
            <div className={`flex items-center gap-4 border-b pb-4 mb-12 ${theme.border}`}>
              ...
              <h2 className={`text-3xl font-bold ${theme.textTitle}`}>Mes Compétences</h2>
            </div>
          </RevealOnScroll>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {skills.map((skill, index) => (
              <RevealOnScroll key={index} delay={index * 100}>
                <div className={`${theme.cardBg} p-6 rounded-2xl border ${theme.border} hover:border-cyan-400 transition-all duration-300 hover:-translate-y-2 ${theme.shadow} group cursor-default`}>
                  <div className="flex items-center gap-4 mb-6">
                    <div className={`p-3 rounded-lg border ${theme.border} ${theme.iconBg} group-hover:border-cyan-400 group-hover:bg-cyan-500/10 transition-all duration-300`}>
                      {skill.icon}
                    </div>
                    <h3 className={`text-xl font-bold group-hover:text-cyan-400 transition-colors ${theme.textTitle}`}>{skill.category}</h3>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {skill.items.map((item, idx) => (
                      <span 
                        key={idx} 
                        className={`px-3 py-1.5 rounded-lg text-sm border transition-all duration-300 transform hover:scale-105 ${isDarkMode ? 'bg-cyan-900/20 text-cyan-200 border-cyan-800/30 hover:bg-cyan-500 hover:text-white' : 'bg-cyan-50 text-cyan-700 border-cyan-100 hover:bg-cyan-500 hover:text-white'}`}
                      >
                        {item}
                      </span>
                    ))}
        ...
        <section id="projects" className="py-20 scroll-mt-20">
          <RevealOnScroll>
            <div className={`flex items-center gap-4 border-b pb-4 mb-12 ${theme.border}`}>
              ...
              <h2 className={`text-3xl font-bold ${theme.textTitle}`}>Projets</h2>
            </div>
          </RevealOnScroll>
          ...
          <div className="grid md:grid-cols-1 lg:grid-cols-2 gap-8">
            {projects.map((project, index) => (
              <RevealOnScroll key={index} delay={index * 100}>
                <div className={`group relative ${theme.cardBg} rounded-2xl overflow-hidden border ${theme.border} hover:border-cyan-500 transition-all duration-300 flex flex-col h-full hover:shadow-2xl ${theme.shadow} hover:-translate-y-2`}>
                  ...
                  <div className="p-8 flex flex-col flex-grow relative z-10">
                    <div className="flex justify-between items-start mb-4">
                      <h3 className={`text-2xl font-bold group-hover:text-cyan-400 transition-colors ${theme.textTitle}`}>{project.title}</h3>
                    </div>
                    <span className={`inline-block w-fit mb-4 text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider transition-colors ${isDarkMode ? 'bg-slate-700 text-cyan-300 group-hover:bg-cyan-900/30' : 'bg-slate-100 text-cyan-600 group-hover:bg-cyan-50'}`}>
                      {project.type}
                    </span>
                    <p className={`${theme.textMuted} mb-6 leading-relaxed flex-grow text-sm group-hover:text-slate-500 dark:group-hover:text-slate-300 transition-colors`}>
                      {project.description}
                    </p>
                    <div className="mt-auto">
                      <div className="flex flex-wrap gap-2 mb-6">
                        {project.tags.map((tag, i) => (
                          <span key={i} className={`text-xs border px-2 py-1 rounded transition-colors ${isDarkMode ? 'text-slate-300 bg-slate-900 border-slate-700 group-hover:border-cyan-500/50' : 'text-slate-600 bg-slate-100 border-slate-200 group-hover:border-cyan-500/50'}`}>#{tag}</span>
                        ))}
                      </div>
                      {project.link !== "#" ? (
        ...
        <section id="experience" className="py-20 scroll-mt-20">
          <RevealOnScroll>
            <div className={`flex items-center gap-4 border-b pb-4 mb-12 ${theme.border}`}>
              ...
              <h2 className={`text-3xl font-bold ${theme.textTitle}`}>Parcours Professionnel</h2>
            </div>
          </RevealOnScroll>
            ...
            {experiences.map((exp, index) => (
              <RevealOnScroll key={index} delay={200}>
                <div className="relative pl-0 md:pl-16 group">
                  <div className={`hidden md:block absolute left-[18px] top-8 w-5 h-5 rounded-full border-4 border-cyan-500 z-10 group-hover:scale-125 transition-transform duration-300 ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}></div>
                  
                  <div className={`${theme.cardBg} p-8 rounded-2xl border ${theme.border} shadow-lg ${theme.shadow} transition-all hover:border-cyan-500/50`}>
                    <div className={`flex flex-col md:flex-row md:justify-between md:items-center mb-6 border-b pb-4 ${theme.border}`}>
                      <div>
                        <h3 className={`text-2xl font-bold group-hover:text-cyan-400 transition-colors ${theme.textTitle}`}>{exp.role}</h3>
                        <h4 className="text-lg text-cyan-500 font-medium">{exp.company}</h4>
                      </div>
                      <span className={`mt-2 md:mt-0 px-4 py-1 rounded-full text-sm font-mono border ${isDarkMode ? 'bg-cyan-900/30 text-cyan-200 border-cyan-800/30' : 'bg-cyan-50 text-cyan-700 border-cyan-200'}`}>
                        {exp.period}
                      </span>
                    </div>
                    
                    <p className={`${theme.textMuted} mb-6 italic text-lg`}>
                      {exp.description}
                    </p>
                    ...
                    <div className="grid md:grid-cols-1 gap-3">
                      {exp.details.map((detail, i) => (
                        <div key={i} className="flex gap-3 items-start">
                          ...
                          <p className={`${theme.textMuted} text-sm leading-relaxed`}>{detail}</p>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <section id="contact" className="py-20 scroll-mt-20 mb-20">
          <RevealOnScroll>
            <div className={`flex items-center gap-4 border-b pb-4 mb-12 ${theme.border}`}>
              ...
              <h2 className={`text-3xl font-bold ${theme.textTitle}`}>Me Contacter</h2>
            </div>
          </RevealOnScroll>
          
          <RevealOnScroll delay={200}>
            <div className={`${theme.cardBg} p-8 rounded-2xl border ${theme.border} shadow-2xl relative overflow-hidden group hover:border-cyan-500/50 transition-colors`}>
              ...
              <p className={`${theme.textMuted} mb-8 text-center text-lg`}>
              ...
              <form action="https://formspree.io/f/xnndawpa" method="post" className="space-y-6 max-w-2xl mx-auto relative z-10">
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <label htmlFor="nom" className={`block text-sm font-medium mb-2 ${theme.textTitle}`}>Nom</label>
                    <input 
                      type="text" 
                      id="nom" 
                      name="nom" 
                      required
                      className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 transition-all ${theme.inputBg} ${theme.border} ${theme.textTitle}`}
                      placeholder="Votre nom"
                    />
                  </div>
                  <div>
                    <label htmlFor="email" className={`block text-sm font-medium mb-2 ${theme.textTitle}`}>Email</label>
                    <input 
                      type="email" 
                      id="email" 
                      name="email" 
                ...
                <div>
                  <label htmlFor="message" className={`block text-sm font-medium mb-2 ${theme.textTitle}`}>Message</label>
                  <textarea 
                    id="message" 
                    name="message" 
                    rows="5" 
                    required
                    className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 transition-all resize-none ${theme.inputBg} ${theme.border} ${theme.textTitle}`}
              ...
              <div className={`mt-12 flex justify-center gap-8 pt-8 border-t ${theme.border}`}>
                <a href="mailto:cheicksow384@gmail.com" className={`flex flex-col items-center gap-2 ${theme.textMuted} hover:text-cyan-500 transition-colors group`}>
                  <div className={`p-4 rounded-full border ${theme.border} ${theme.iconBg} group-hover:border-cyan-400 group-hover:bg-cyan-500/10 transition-all transform group-hover:-translate-y-2`}>
                    <Mail size={24} />
                  </div>
                  <span className="text-sm font-medium">Email</span>
                </a>
                <a href="https://www.linkedin.com/in/cheick-oumar-sow-6434192ab" target="_blank" rel="noopener noreferrer" className={`flex flex-col items-center gap-2 ${theme.textMuted} hover:text-cyan-500 transition-colors group`}>
                  <div className={`p-4 rounded-full border ${theme.border} ${theme.iconBg} group-hover:border-cyan-400 group-hover:bg-cyan-500/10 transition-all transform group-hover:-translate-y-2`}>
                    <Linkedin size={24} />
                  </div>
                  <span className="text-sm font-medium">LinkedIn</span>
                </a>
                <a href="https://github.com/Cheick6" target="_blank" rel="noopener noreferrer" className={`flex flex-col items-center gap-2 ${theme.textMuted} hover:text-cyan-500 transition-colors group`}>
                  <div className={`p-4 rounded-full border ${theme.border} ${theme.iconBg} group-hover:border-cyan-400 group-hover:bg-cyan-500/10 transition-all transform group-hover:-translate-y-2`}>
                    <Github size={24} />
                  </div>
      ...
      {/* Footer */}
      <footer className={`relative z-10 py-8 border-t ${theme.border} ${theme.navBg} text-center`}>
        <p className={`${theme.textMuted} text-sm`}>
          &copy; {new Date().getFullYear()} Cheick-Oumar Sow. Tous droits réservés.
        </p>
        <p className={`${theme.textMuted} text-xs mt-2 flex items-center justify-center gap-1`}>
